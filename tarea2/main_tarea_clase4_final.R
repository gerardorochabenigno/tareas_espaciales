...
###################################################################################
#### Esto ya no va, pero está interesante 

library(sf)
library(dplyr)
library(ggplot2)

# --- Supuestos previos: ya tienes denue_norm y fsq_norm con columnas estándar ---
# denue_norm: id | name_original | name_norm | latitude | longitude
# fsq_norm:   id | name_original | name_norm | latitude | longitude

# Elegimos 1 registro de DENUE a modo de ejemplo (puedes cambiar el id)
target_denue_id <- denue_norm$id[1]

# 1) Convertir a sf en 4326
denue_sf <- st_as_sf(denue_norm, coords = c("longitude", "latitude"), crs = 4326)
fsq_sf   <- st_as_sf(fsq_norm,   coords = c("longitude", "latitude"), crs = 4326)

# 2) Obtener el punto objetivo (DENUE) y proyectar a 3857 para crear buffers en metros
denue_pt_4326 <- denue_sf %>% filter(id == target_denue_id)
denue_pt_3857 <- st_transform(denue_pt_4326, 3857)

# 3) Crear buffers de 200 m y 400 m en 3857, y regresar a 4326 para graficar
buf200_3857 <- st_buffer(denue_pt_3857, dist = 200)
buf400_3857 <- st_buffer(denue_pt_3857, dist = 400)
buf200_4326 <- st_transform(buf200_3857, 4326)
buf400_4326 <- st_transform(buf400_3857, 4326)

# 4) Seleccionar FSQ dentro de 400 m (máximo radio de este ejemplo)
#    Usamos 3857 para la prueba de intersección (más preciso con buffers en metros)
fsq_3857 <- st_transform(fsq_sf, 3857)
inside_400_idx <- st_within(fsq_3857, buf400_3857) %>% lengths() > 0
fsq_in_400 <- fsq_sf[inside_400_idx, ]

# 5) (Opcional) colorear los FSQ según si caen en 200 m o solo en 200–400 m
inside_200_idx <- st_within(fsq_3857, buf200_3857) %>% lengths() > 0
fsq_sf$ring <- dplyr::case_when(
  inside_200_idx ~ "≤ 200 m",
  inside_400_idx ~ "200–400 m",
  TRUE           ~ "> 400 m"
)
fsq_in_400 <- fsq_sf %>% filter(ring %in% c("≤ 200 m", "200–400 m"))

# 6) Plot con ggplot2 (DENUE, buffers y FSQ cercanos)
ggplot() +
  # buffers (de mayor a menor para que se vea el borde)
  geom_sf(data = buf400_4326, fill = NA, linewidth = 0.6, linetype = "dashed") +
  geom_sf(data = buf200_4326, fill = NA, linewidth = 0.8, color = "red") +
  # punto DENUE
  geom_sf(data = denue_pt_4326, size = 3) +
  # puntos FSQ dentro de 400 m, coloreados por anillo
  geom_sf(data = fsq_in_400, aes(color = ring), alpha = 0.9) +
  # estilos
  labs(
    title    = "Buffers en metros alrededor de un punto DENUE",
    subtitle = "Círculos de 200 m (rojo) y 400 m (trazo) en EPSG:3857, graficados en 4326",
    x = "Longitud", y = "Latitud", color = "Distancia"
  ) +
  theme_minimal()
